...
//   - co-located (มี STORAGE_ID): หยิบเฉพาะงานที่ enqueuer ประทับ
//     targetStorageId = storage ของเครื่องนี้
//   - remote pool (ไม่มี STORAGE_ID): หยิบเฉพาะงานที่ไม่ผูก storage
//     (enqueuer จ่ายแบบไม่ใส่ targetStorageId ให้ pool กลาง)
//
// Returns (nil, nil) when the queue is empty.
func Claim(ctx context.Context, workerID string) (*models.VideoProcess, error) {
	now := time.Now()
	filter := bson.M{
		"processType": enums.ProcessTypeSpritesheet,
