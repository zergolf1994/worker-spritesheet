...
func RunLoop(ctx context.Context, workerID string, handler JobHandler) {
	...
	for {
		...
		// kill switch (spritesheet_config.enabled) — shared with the enqueuer
		if !spritesheetEnabled(ctx) {
...
func runJob(ctx context.Context, workerID string, job *models.VideoProcess, handler JobHandler) {
	...
	// Update the dashboard immediately. Heartbeat is only sent once a minute,
	// so a short job could otherwise start and finish without ever showing busy.
...
// spritesheetEnabled reads spritesheet_config.enabled — missing/malformed = true
// (fail-open: a broken settings doc must not silently stop every worker).
func spritesheetEnabled(ctx context.Context) bool {
	setting, err := models.SettingModel.FindOne(ctx, bson.M{"name": enums.SettingSpritesheetConfig})
	if err != nil {
		if !errors.Is(err, mongo.ErrNoDocuments) && ctx.Err() == nil {
			log.Printf("⚠️ Read spritesheet_config failed: %v", err)
		}
		return true
	}
