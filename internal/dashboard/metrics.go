...
func sampleGPUs() ([]GPUInfo, string) {
	...
	for _, row := range records {
		...
		gpus = append(gpus, GPUInfo{Name: strings.TrimSpace(row[1]), GPUPercent: number(row[2]), EncoderPercent: codec.encoder, DecoderPercent: codec.decoder, MemoryUsed: int64(number(row[3])) * 1024 * 1024, MemoryTotal: int64(number(row[4])) * 1024 * 1024, Temperature: number(row[5]), PowerWatts: number(row[6])})
	}
	if len(gpus) == 0 {
