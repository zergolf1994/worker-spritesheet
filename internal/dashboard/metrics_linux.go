...
func (s *platformSampler) sample() (CPUInfo, MemoryInfo, DiskInfo) {
	...
	memory := readMemory()
	total, used := diskUsage(s.storagePath)
...
func readDiskIO() ioCounters {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		item := row{name: name, read: reads * 512, write: writes * 512}
		all = append(all, item)
	...
	for _, item := range selected {
		...
		result.write += item.write
	}
	return result
}
func diskUsage(path string) (int64, int64) {
	var stat syscall.Statfs_t
	if syscall.Statfs(path, &stat) != nil && syscall.Statfs("/", &stat) != nil {
		return 0, 0
	}
